...
<template>
    <li class="list-group-item" :class="{'bg-dark': !selected, 'bg-white': selected, 'text-white': !selected, 'text-dark': selected}">
        <div>
          <h6 class="card-text" :class="{'truncate': !expand && headline.body.length > 500}"><span :class="{'text-info': !selected, 'text-primary': selected}" v-show="headline.type == 'twitter'"><i class="bi bi-twitter"></i> {{ headline.title }}: </span>{{ headline.body }}&nbsp;<a class="float-right" v-show="headline.link" :href="headline.link" target="_blank">Link</a></h6>
